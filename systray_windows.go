...
func registerSystray() {
	if err := wt.initInstance(); err != nil {
		logger.Printf("Unable to init instance: %v", err)
		return
	}

	if err := wt.createMenu(); err != nil {
		logger.Printf("Unable to create menu: %v", err)
		return
	}
...
func nativeLoop() {
	...
	for {
		...
		switch int32(ret) {
		case -1:
			logger.Printf("Error at message loop: %v", err)
			return
		case 0:
			return
		default:
...
func SetIcon(iconBytes []byte) {
	...
	if err != nil {
		logger.Printf("Unable to write icon data to temp file: %v", err)
		return
	}
	if err := wt.setIcon(iconFilePath); err != nil {
		logger.Printf("Unable to set icon: %v", err)
		return
	}
}
...
func (item *MenuItem) SetIcon(iconBytes []byte) {
	...
	if err != nil {
		logger.Printf("Unable to load icon from temp file: %v", err)
		return
	}

	h, err = wt.iconToBitmap(h)
	if err != nil {
		logger.Printf("Unable to convert icon to bitmap: %v", err)
		return
	}
	...
	if err != nil {
		logger.Printf("Unable to addOrUpdateMenuItem: %v", err)
		return
	}
}
...
func SetTooltip(tooltip string) {
	if err := wt.setTooltip(tooltip); err != nil {
		logger.Printf("Unable to set tooltip: %v", err)
		return
	}
}
...
func addSeparator(id uint32) {
	...
	if err != nil {
		logger.Printf("Unable to addSeparator: %v", err)
		return
	}
}
...
func hideMenuItem(item *MenuItem) {
	...
	if err != nil {
		logger.Printf("Unable to hideMenuItem: %v", err)
		return
	}
}
